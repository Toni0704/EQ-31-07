...
class BiLstmModel(nn.Module):
    ...
    def _create_network(self):
        ...
        self.output_net = nn.Sequential(
            nn.Linear(self.filter_arrays[-1] * 2, 256), # 256 only for synthetic dataset, otherwise 64
            nn.ReLU(),
            nn.Linear(256, self.output_size), # 256 only for synthetic dataset, otherwise 64
        )
