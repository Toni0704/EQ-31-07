...
from src.interpretability.manipulation_results_percentile import ScoreComputation
...
def evaluate_interpretability(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    with open(os.path.join(cfg.paths.output_dir,"nounid.npy"), 'wb') as f:
        np.save(f, np_nounid)
    with open(os.path.join(cfg.paths.output_dir,"fsum.npy"), 'wb') as f:
        np.save(f, np_fsum)
    ...
    relevance_path = cfg.relevance_path
    relevance_paste_path = os.path.join(path_results, "interpretability_results/interpretability_raw/")
    # os.popen(f"cp -r {relevance_path} {relevance_paste_path}")
    shutil.copytree(relevance_path, relevance_paste_path)
...
@hydra.main(version_base="1.3", config_path="../configs", config_name="eval_interpret_percentile.yaml")
