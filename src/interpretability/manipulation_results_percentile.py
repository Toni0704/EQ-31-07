...
from src.datamodules.time_datamodule import TimeDataModule as DataModule
from src.models.augmentation.transform_signal import random_block_enforce_proba
from interpretability.components.method_arguments import dict_method_arguments
...
class ScoreComputation:
    """
    Main class to compute the different key interpretability metrics
    ...
    def __init__(
        ...
        ----------
        model: nn.Module
            Customed model for training process.
        signal: np.array
            Time series data set for interpretability and evaluation.
        target: np.array
            Labels/Classes corresponding to time series.
        nounid: np.array
            Sample names corresponding to time series.
        path_results: str
            Path to the folder where the results are stored.
        model_output: "probabilities" or "logits"
            Two options - 1) Post-SoftMax: "probabilities"; 2) Pre-SoftMax: "logits".
        """

        self.model = model.to(device)
        self.signal = signal # dim of signal: (instances, nb_features, time-step)
        self.nb_features = signal.shape[1]
        self.target = target
        self.list_sample_names = nounid.tolist()
        self.list_signal_names = [np.array([f'feature_{str(x)}' for x in range(self.nb_features)]).astype(str).tolist()]*signal.shape[0]
        # self.plot_signal = plot_signal
        # if self.plot_signal is False or self.plot_signal is None:
        #     self.plot_signal = 0
        self.model_output = model_output
        self.encoder = load(open(f"{path_results}/datamodule/target_encoder.pkl", "rb"))
        self.save_results = os.path.join(path_results, "interpretability_results")

        if not os.path.exists(self.save_results):
            os.makedirs(self.save_results)

        if self.model_output == "probabilities":
            print("Model output is Post-SoftMax(probability).")
            self.model = nn.Sequential(self.model, nn.Softmax(dim=-1)).eval()
        elif self.model_output == "logits":
            print("Model output is Pre-SoftMax(logits).")
        else:
            raise ValueError("Model_output must be either probability or logits")
        
        self.baseline = self.signal.copy()
        score_pred = self.__make_predictions(self.signal)
    ...
    #     if dict_method_arguments.get(method_relevance) is None:
    #         raise ValueError(
    #             f"{method_relevance} \
    ...
    #     # set up device for lstm model and multi GPUs for some interpretability methods
    #     lstm_network_bool = any(
    #         [isinstance(module, torch.nn.modules.rnn.LSTM) for module in self.model.modules()]
    #     )
    #     global device_interpretability
    #     if (lstm_network_bool) & (dict_method_arguments[method_relevance]["noback_cudnn"]):
    #         print("Attribution for LSTM based network is not supported on GPU. Switching to CPU")
    #         device_interpretability = torch.device("cpu")
    #         self.model.to(device_interpretability)
    #     else:
    #         device_interpretability = device
    #         self.model.to(device_interpretability)

    #     # if dict_method_arguments[method_relevance]["noback_cudnn"]:
    #     #     device_interpretability = torch.device("cpu")
    #     #     self.model.to(device_interpretability)
    #     # else:
    #     #     device_interpretability = device
    ...
    #     batch_size = dict_method_arguments[method_relevance].get("batch_size")
    ...
    #     batched_sample = np.array_split(self.signal, np.ceil(self.signal.shape[0] / batch_size))
    #     batched_idx = np.array_split(pred_class_idx, np.ceil(self.signal.shape[0] / batch_size))
    ...
    #     rel_method = rel_method(self.model, **(kwargs_method))

    #     kwargs_attribution = dict_method_arguments[method_relevance].get("kwargs_attribution", {})

    #     bool_has_convergence = rel_method.has_convergence_delta()
    #     if bool_has_convergence:
    #         # kwargs_attribution["return_convergence_delta"] = True
    #         kwargs_attribution["return_convergence_delta"] = False
    ...
    #         )
    #         kwargs_attribution["baselines"] = baseline.to(device_interpretability)
    ...
    #     ):
    #         results_relevance = attribute_series_features(
    #             rel_method,
    #             self.model,
    #             torch.Tensor(sample).to(device_interpretability),
    #             target_idx,
    #             **(kwargs_attribution),
    #         )

    #         # if bool_has_convergence:
    #         #     tmp_rel, delta = results_relevance
    #         #     if torch.sum(delta) > 0.1:
    #         #         print(f"Relevance delta for method {method_relevance} is {delta}.")
    #         #     del delta
    #         #     gc.collect()
    #         # else:
    #         #     tmp_rel = results_relevance
    #         tmp_rel = results_relevance
    ...
    #     np_rel = torch_rel.cpu().detach().numpy() # [batch, nb_features, time-step]
    ...
    #     # save the relevance as .csv files
    #     print("Converting to [batch, time-step, feature] format")
    #     np_rel = np.transpose(np_rel, (0, 2, 1)) # [batch, time-step, nb_features]
    #     # Format DataFrame for relevance 
    #     np_rel_df = np_rel.transpose((1, 0, 2)).reshape((np_rel.shape[1], -1)) # [time-step, nb_features*batch]

    #     df_relevance = pd.DataFrame(np_rel_df)

    #     names_all = np.repeat(self.list_sample_names, self.nb_features)
    #     list_signal_names_flat = [item for sublist in self.list_signal_names for item in sublist]
    #     names_columns = [
    ...
    def __compute_scores(
        ...
        df_results = pd.DataFrame()
        np_all_modified_signal = np.empty(self.signal.shape)
        np_all_randomly_modified_signal = np.empty(self.signal.shape)

        # Iterate across all samples to compute metrics
        for idx in tqdm(range(self.signal.shape[0])):
            pred_idx = self.pred_idx[idx]
            name_sample = self.list_sample_names[idx]
            signal_names = self.list_signal_names[idx]
            signal = self.signal[idx, :, :].copy()
    ...
    def __interpretability_metrics(
        ...
        pred_idx: numpy array
            pred model encoded
        interp: numpy array [time-step,nb_features]
            relevance scores
        path_save: str
        ...
        index_percentile = int(k_feature * sp_interp_all.shape[0] - 1)
        if topk:
            # get top k-percentile time steps
            
            sp_interp_all_sorted = np.sort(sp_interp_all)[::-1]
            thres_p = sp_interp_all_sorted[index_percentile]
            # quantile = 1 - k_feature
            # thres_p = np.quantile(sp_interp_all, quantile)
        else:
            # get bottom k-percentile time steps
            sp_interp_all_sorted = np.sort(sp_interp_all)
            thres_p = sp_interp_all_sorted[index_percentile]
            # quantile = k_feature
            # thres_p = np.quantile(sp_interp_all, quantile)
    ...
    def __compute_score_pred(
        ...
        batched_sample = np.array_split(
            self.signal, np.ceil(self.signal.shape[0] / batch_size)
        )
        ...
        df_score.loc[:, "score1"] = np.array([np.max(x) for x in score1])
        df_score.loc[:, "initial_classification"] = correct_classification

        batched_sample = np.array_split(
            np_all_modified_signal, np.ceil(self.signal.shape[0] / batch_size)
        )
        ...
        df_score.loc[:, "score3"] = np.array([np.max(x) for x in score3])
        df_score.loc[:, "random_classification"] = correct_classification

        sample_expected_values = self.expected_value * np.asarray(self.target)
    ...
    def __make_predictions(self, sample: np.array):
        """Get the prediction of input signals.

        Args:
            sample (np.array): Signals

        Returns:
            score_pred (tensor): Predictions corresponding to signals.
        """
        batched_sample = np.array_split(sample, np.ceil(sample.shape[0] / 16))
        score_pred = torch.tensor([], device=device, requires_grad=False)
        for signal in batched_sample:
            with torch.no_grad():
                score_pred = torch.cat(
                    (score_pred, self.model(torch.Tensor(signal).to(device)))
                )
